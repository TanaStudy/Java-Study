...
export default {
    ...
    getSpuBaseAttrs() {
      this.$http({
        url: this.$http.adornUrl(
          `/product/productattrvalue/listforspu/${this.spuId}`
        ),
        method: "get",
      }).then(({ data }) => {
    ...
    submitSpuAttrs() {
      ...
      this.$confirm("修改商品规格信息, 是否继续?", "提示", {
        ...
        .then(() => {
          this.$http({
            url: this.$http.adornUrl(
              `/product/productattrvalue/update/${this.spuId}`
            ),
            method: "post",
  ...
  activated() {
    ...
    if (this.spuId && this.catelogId) {
      console.log("actived", this.spuId, this.catelogId);
      this.showBaseAttrs();
